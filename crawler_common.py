...
# Windows Chrome major must stay aligned with curl_cffi chrome146 when used as fallback.
USER_AGENTS = [
...
def sync_path_exists(session, url, baseline_length, baseline_status, bypass_forbidden=True):
    try:
        # GET-only existence probe — avoids Akamai "HTTP HEAD Method Used" bot rule.
