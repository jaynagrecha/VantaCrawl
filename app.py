...
from crawler_common import DownloadManager, get_project_paths, get_random_user_agent, is_html_url, normalize_extensions
...
def _get_selenium_driver(proxy_url=""):
...
def _fetch_with_selenium(url, settle_seconds=2, proxy_url="", screenshot_path=None):
    from selenium.webdriver.common.by import By
    from selenium.webdriver.support import expected_conditions as EC
    from selenium.webdriver.support.ui import WebDriverWait

    driver = _get_selenium_driver(proxy_url)
    driver.get(url)
    try:
...
class CrawlerApp(QMainWindow):
    ...
    def apply_selenium_login(self, config):
        ...
        self.output_text.append("Running browser login to capture cookies…")
        cookies, message = selenium_login(
            config.login_url,
            config.login_username,
            config.login_password,
            driver_factory=lambda: _get_selenium_driver(config.proxy_url),
        )
